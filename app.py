...
# Load comparison metrics (precomputed)
...
if os.path.exists(metrics_path):
    ...
    st.warning("Run evaluation first to generate metrics_summary.csv")
...
chosen_label = st.selectbox("Select a model", list(model_map.keys()))
model_file = model_map[chosen_label]

# Upload CSV
uploaded = st.file_uploader("Upload test CSV (must include 'label' column)", type=["csv"])
...
if TARGET_COL not in df.columns:
    st.error("Uploaded file must include a 'label' column.")
    st.stop()
...
if missing:
    st.error(f"Missing required feature columns: {missing[:5]} ...")
    st.stop()
X = X[feature_cols]
...
st.subheader(f"Metrics for: {chosen_label}")
